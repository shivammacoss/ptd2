...
export default function ChargesPage() {
  ...
  const selectUser = (idx: number, user: { id: string; name: string; email: string }) => {
    updateRow(idx, 'user_id', user.id);
    setRows(prev => prev.map((r, i) => i === idx ? { ...r, user_id: user.id, _user_label: `${user.name} (${user.email})` } : r));
    setUserSearchIdx(null);
    setUserSearchQuery('');
  ...
  const getRowIdx = (r: ChargeRow) => rows.indexOf(r);

  function RuleTable({ title, items, scopeType }: { title: string; items: ChargeRow[]; scopeType: string }) {
    return (
                ...
                return (
                  <tr key={idx} className="border-b border-border-primary/50 hover:bg-bg-hover/30">
                      ...
                      <td className="px-3 py-2">
                        {r._user_label ? (
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => { updateRow(idx, 'user_id', null); setRows(prev => prev.map((x, i) => i === idx ? { ...x, _user_label: undefined } : x)); }} className="text-text-tertiary hover:text-danger"><X size={10} /></button>
                          </div>
                        ) : (
                            ...
                            {userSearchIdx === idx && userSearchResults.length > 0 && (
                              <div className="absolute left-0 right-0 top-full mt-1 z-20 max-h-32 overflow-y-auto border border-border-primary rounded bg-bg-secondary shadow-dropdown">
                                {userSearchResults.map(u => (
                                  <button key={u.id} onClick={() => selectUser(idx, u)} className="w-full text-left px-2 py-1.5 text-xxs hover:bg-bg-hover border-b border-border-primary/50 last:border-0">
                    ...
                    {(scopeType === 'instrument' || scopeType === 'user') && (
                      <td className="px-3 py-2">
                        <select value={r.instrument_id || ''} onChange={e => updateRow(idx, 'instrument_id', e.target.value || null)} className="text-xs py-1 pl-2 pr-6 appearance-none bg-bg-input border border-border-primary rounded text-text-primary w-32">
                    ...
                    <td className="px-3 py-2">
                      <select value={r.charge_type} onChange={e => updateRow(idx, 'charge_type', e.target.value)} className="text-xs py-1 pl-2 pr-6 appearance-none bg-bg-input border border-border-primary rounded text-text-primary w-32">
                        {CHARGE_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                      </select>
                    </td>
                    <td className="px-3 py-2">
                      <input type="number" step="any" min="0" value={r.value} onChange={e => updateRow(idx, 'value', parseFloat(e.target.value) || 0)} className="w-20 px-2 py-1 text-xs bg-bg-input border border-border-primary rounded font-mono tabular-nums text-text-primary" />
                    </td>
                    <td className="px-3 py-2">
                      <button onClick={() => updateRow(idx, 'is_enabled', !r.is_enabled)} className={cn('w-8 h-4 rounded-full transition-fast relative', r.is_enabled ? 'bg-buy' : 'bg-bg-hover border border-border-primary')}>
                        <span className={cn('absolute top-0.5 w-3 h-3 rounded-full bg-white shadow transition-fast', r.is_enabled ? 'left-[16px]' : 'left-0.5')} />
                      </button>
                    </td>
                    <td className="px-3 py-2">
                      <button onClick={() => removeRow(idx)} className="p-1 text-text-tertiary hover:text-danger transition-fast"><Trash2 size={12} /></button>
                    </td>
                  </tr>
                );
              })}
      ...
      <div className="p-6 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xxs text-text-tertiary mt-0.5">Priority: User &gt; Instrument &gt; Default. Higher priority overrides lower.</p>
          </div>
