...
export default function UsersPage() {
  ...
  const submitFundAction = async () => {
    ...
    if (!modalAccountId) { toast.error('Select an account'); return; }
      ...
      <Modal open={isFundAction(modalType)} onClose={closeModal} title={modalType && isFundAction(modalType) ? `${FUND_LABELS[modalType]} — ${modalUser?.name}` : ''}>
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea value={modalReason} onChange={e => setModalReason(e.target.value)} rows={3} placeholder="Optional reason..." className="w-full px-4 py-3 text-sm bg-bg-input border border-border-primary rounded-lg placeholder:text-text-tertiary focus:border-buy transition-fast resize-none" />
          </div>
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="button" onClick={submitFundAction} disabled={modalSubmitting || !modalAccountId} className={cn('px-5 py-2.5 rounded-lg text-sm font-medium transition-fast inline-flex items-center gap-2', 'bg-buy text-white hover:bg-buy-light disabled:opacity-50')}>
