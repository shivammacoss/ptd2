    ...
    <div className="flex flex-col h-screen bg-bg-primary">
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto p-4 md:p-6 space-y-6">
          <h2 className="text-lg font-semibold text-text-primary">Business</h2>
          <div className="flex gap-1 glass-card rounded-full px-1.5 py-1 w-fit">
            {TABS.map(t => (
              <button key={t.id} onClick={() => setTab(t.id)} className={clsx('px-3 py-1.5 text-xs font-medium rounded-full transition-all', tab === t.id ? 'skeu-btn-buy text-text-inverse' : 'text-text-secondary hover:text-text-primary')}>
                {t.label}
              </button>
            ))}
          </div>
