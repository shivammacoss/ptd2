...
CREATE TABLE kyc_documents (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID REFERENCES users(id) ON DELETE CASCADE,
    document_type VARCHAR(30) NOT NULL CHECK (document_type IN ('id_front', 'id_back', 'address_proof', 'selfie', 'other')),
...
INSERT INTO instruments (symbol, display_name, segment_id, base_currency, quote_currency, digits, pip_size) VALUES
    ...
    ('USOIL', 'US Crude Oil', (SELECT id FROM instrument_segments WHERE name='commodities'), 'OIL', 'USD', 2, 0.01),
...
CREATE TABLE charge_configs (
    ...
    charge_type VARCHAR(30) NOT NULL CHECK (charge_type IN ('commission_per_lot', 'commission_per_trade', 'spread_percentage')),
...
CREATE TABLE spread_configs (
    ...
    instrument_id UUID REFERENCES instruments(id),
    user_id UUID REFERENCES users(id),
    spread_type VARCHAR(20) NOT NULL CHECK (spread_type IN ('fixed', 'variable')),
    value DECIMAL(18,8) NOT NULL,
    is_enabled BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
...
CREATE TABLE deposits (
    ...
    method VARCHAR(30) CHECK (method IN ('bank_transfer', 'upi', 'qr', 'crypto_btc', 'crypto_eth', 'crypto_usdt', 'metamask')),
...
CREATE TABLE withdrawals (
    ...
    amount DECIMAL(18,8) NOT NULL,
    currency VARCHAR(10) DEFAULT 'INR',
    method VARCHAR(30) CHECK (method IN ('bank_transfer', 'upi', 'crypto_btc', 'crypto_eth', 'crypto_usdt', 'metamask')),
...
VALUES ('admin@protrader.com', '$2b$12$LJ3m4ys3YOlPNZQRFMJnDuLWZMCjWl.VJ0OTc/3hxJ7G7FzBBNzOa', 'Super', 'Admin', 'super_admin', 'active', 'approved');
