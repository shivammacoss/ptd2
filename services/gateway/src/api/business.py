...
async def ib_status(
    ...
    profile_result = await db.execute(
        select(IBProfile).where(IBProfile.user_id == current_user["user_id"])
    )
    profile = profile_result.scalar_one_or_none()

    app_result = await db.execute(
        select(IBApplication).where(IBApplication.user_id == current_user["user_id"])
...
async def apply_ib(
    ...
    existing_app = await db.execute(
        select(IBApplication).where(
            IBApplication.user_id == current_user["user_id"],
